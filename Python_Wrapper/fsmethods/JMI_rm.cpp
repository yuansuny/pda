...
//		}
		...
		for (int i = 0; i < Nfeatures; ++i) {
            if (feature_set.find(i) == feature_set.end()){
                ...
                for (int j = 0; j < Nsamples; ++j) {
                    ...
                    int secondValue = distribution.secondStateMap[featureDistribution[selected_feature].firstStateMap[data[j][selected_feature]]+ featureDistribution[i].secondStateMap[label_vector[j]]*featureDistribution[selected_feature].numFirstStates];
                    int secondValue1 = distribution.secondStateMap[featureMergeArray[selected_feature][j]];
                    assert(secondValue == secondValue1);
                    ...
                    int firstValue1 = distribution1.firstStateMap[data[j][i]];
                    secondValue1 = distribution1.secondStateMap[data[j][selected_feature]];
                    double conditional_prob1 = probability_con(distribution1, firstValue1, secondValue1);
                    assert(conditional_prob1 != 0);
                    x_matrix[i][j] = x_matrix[i][j]*(double)initial_step/(double)(initial_step+1);
                    x_matrix[i][j] += conditional_prob1/(double)(initial_step+1);
                }
            }
        }
    ...
    strcat(results,"selFeatures_JMIrm_dataset_");
...
void JMIrm(int nsF=100, string filename="wine", int nS = 10, int nF = 10, int nC = 10){
...
BOOST_PYTHON_FUNCTION_OVERLOADS(JMIrm_overloads, JMIrm, 1, 5)

BOOST_PYTHON_MODULE(JMI_rm){
    boost::python::def("JMIrm", &JMIrm, JMIrm_overloads(boost::python::args("nsF", "filename", "nS", "nF", "nC")));
}

...
void feature_selection_discrete_greedy(int n_features) {
			...
			for (int i = 0; i < Nfeatures; ++i) {
				if (feature_set.find(i) == feature_set.end()){
					...
					for (int j = 0; j < Nsamples; ++j) {
						...
						for (int k = 0; k < featureDistribution[i].numSecondStates; ++k) {
							int firstValue = featureDistribution[i].firstStateMap[data[j][i]];
							int secondValue = distribution.secondStateMap[featureDistribution[last_feature].firstStateMap[data[j][last_feature]]+k*featureDistribution[last_feature].numFirstStates];
							double conditional_prob = probability_con(distribution, firstValue, secondValue);
							memory_store[i][j][k] = memory_store[i][j][k]*(double)(initial_step-1.0)/(double)initial_step;
							memory_store[i][j][k] += conditional_prob/(double)initial_step;
    ...
    strcat(results,"selFeatures_VMIrm_dataset_");
...
void VMIrm(int nsF=100, string filename="wine", int nS = 10, int nF = 10, int nC = 10){
...
BOOST_PYTHON_FUNCTION_OVERLOADS(VMIrm_overloads, VMIrm, 1, 5)

BOOST_PYTHON_MODULE(VMI_rm){
    boost::python::def("VMIrm", &VMIrm, VMIrm_overloads(boost::python::args("nsF", "filename", "nS", "nF", "nC")));
}

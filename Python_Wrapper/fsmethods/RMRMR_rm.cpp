...
void feature_selection_discrete_greedy(int n_features) {
	...
	int cur_step = 0;
	int initial_step = 0;
	double *featureMergeArray1 = new double[Nsamples];
	double conditional_prob, conditional_prob1;
	double max_MI_learned;
	double new_MI_learned;
	int selected_feature;
	int i,j,s;
	int firstValue, firstValue1, secondValue, secondValue1;
		...
		if (initial_step == 0) { //first step: select maximum feature
            for (i = 0; i < Nfeatures; ++i){
                for (j = 0; j < Nsamples; ++j){
                    firstValue = featureDistribution[i].firstStateMap[data[j][i]];
                    secondValue = featureDistribution[i].secondStateMap[label_vector[j]];
...
//		}
		...
		for (i = 0; i < Nfeatures; ++i) {
            if (feature_set.find(i) == feature_set.end()){
                JointProbabilityState distribution = calculateJointProbability(data_inverse[i], featureMergeArray[selected_feature], Nsamples);
                for (j = 0; j < Nsamples; ++j) {
                    firstValue = distribution.firstStateMap[data[j][i]];
                    secondValue = distribution.secondStateMap[featureMergeArray[selected_feature][j]];
                    conditional_prob = probability_con(distribution, firstValue, secondValue);
                    assert(conditional_prob != 0);
                    memory_store[i][j] = memory_store[i][j]*(double)initial_step/(double)(initial_step+1);
                    memory_store[i][j] += conditional_prob/(double)(initial_step+1);
                }

                if (initial_step == 0){
                    JointProbabilityState distribution1 = calculateJointProbability(data_inverse[i], data_inverse[selected_feature], Nsamples);
                    for (j = 0; j < Nsamples; ++j) {
                        firstValue1 = distribution1.firstStateMap[data[j][i]];
                        secondValue1 = distribution1.secondStateMap[data[j][selected_feature]];
                        conditional_prob1 = probability_con(distribution1, firstValue1, secondValue1);
                        assert(conditional_prob1 != 0);
                        x_matrix[i][j] = conditional_prob1;
                    }
                } else{
                    for (j = 0; j <Nsamples; ++j){
                        x_matrix[i][j] = x_matrix[i][j]*(double)(initial_step-1)/(double)(initial_step+1);
                    }
                    for (s = 1; s <= initial_step; ++s){
                        mergeArrays(data_inverse[selected_feature], data_inverse[feature_array[cur_step-s]], featureMergeArray1, Nsamples);
                        JointProbabilityState distribution1 = calculateJointProbability(data_inverse[i], featureMergeArray1, Nsamples);
                        for (j = 0; j < Nsamples; ++j) {
                            firstValue1 = distribution1.firstStateMap[data[j][i]];
                            secondValue1 = distribution1.secondStateMap[featureMergeArray1[j]];
                            conditional_prob1 = probability_con(distribution1, firstValue1, secondValue1);
                            assert(conditional_prob1 != 0);
                            x_matrix[i][j] += 2.0*conditional_prob1/(double)(initial_step+1)/initial_step;
                        }
                    }
                }
            }
    ...
    strcat(results,"selFeatures_RMRMRrm_dataset_");
...
void RMRMRrm(int nsF=100, string filename="wine", int nS = 10, int nF = 10, int nC = 10){
...
BOOST_PYTHON_FUNCTION_OVERLOADS(RMRMRrm_overloads, RMRMRrm, 1, 5)

BOOST_PYTHON_MODULE(RMRMR_rm){
    boost::python::def("RMRMRrm", &RMRMRrm, RMRMRrm_overloads(boost::python::args("nsF", "filename", "nS", "nF", "nC")));
}
